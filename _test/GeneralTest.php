...
final class GeneralTest extends \DokuWikiTest
{
    ...
    public function testPluginInfo(): void
    {
        ...
        self::assertEquals('dx', $info['base']);
    ...
    public function testPluginConf(): void
    {
        ...
        self::assertEquals(
            gettype($conf),
            gettype($meta),
            'Both ' . DOKU_PLUGIN . 'dx/conf/default.php and ' . DOKU_PLUGIN . 'dx/conf/metadata.php have to exist and contain the same keys.'
        );
        ...
        if ($conf !== null && $meta !== null) {
            foreach ($conf as $key => $value) {
                self::assertArrayHasKey(
                    $key,
                    $meta,
                    'Key $meta[\'' . $key . '\'] missing in ' . DOKU_PLUGIN . 'dx/conf/metadata.php'
                );
            }
            ...
            foreach ($meta as $key => $value) {
                self::assertArrayHasKey(
                    $key,
                    $conf,
                    'Key $conf[\'' . $key . '\'] missing in ' . DOKU_PLUGIN . 'dx/conf/default.php'
                );
            }
        }
    }
